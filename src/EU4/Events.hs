...
findTriggeredEventsInOnActions hm scr = foldl' findInAction hm scr
    where
        ...
        actionNameTable = HM.fromList
            ...
            ,("on_conquistador_empty", "<!-- on_conquistador_empty -->{{icon|conquistador}} Conquistador is entering a uncolonized province with ''\"Hunt for the Fragments of the Precursor Empire\"'' mission")
            ,("on_conquistador_native", "<!-- on_conquistador_native -->{{icon|conquistador}} Conquistador is entering a province owned by natives with ''\"Hunt for the Fragments of the Precursor Empire\"'' mission")
            --,("on_death_election", "")
