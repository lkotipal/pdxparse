...
-- data structures.
parseHOI4IdeaGroups :: (IsGameData (GameData g),
                       IsGameState (GameState g),
                       Monad m) =>
    HashMap String GenericScript -> PPT g m IdeaTable
parseHOI4IdeaGroups ideaGroupScripts = do
...
-- appropriate files.
writeHOI4IdeaGroups :: (HOI4Info g, MonadIO m) => PPT g m ()
writeHOI4IdeaGroups = do
...
ppIdeaGroup :: (HOI4Info g, Monad m) => IdeaGroup -> PPT g (ExceptT Text m) Doc
